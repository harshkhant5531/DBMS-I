...
--1. Display all the villages of Rajkot city.  
		...
		SELECT Village.Name FROM CITY JOIN Village ON 
		City.CityID = Village.CityID 
		WHERE City.Name = 'RAJKOT';

--2. Display city along with their villages & pin code.

		SELECT CITY.Name,Village.Name,City.Pincode FROM CITY JOIN Village ON 
		City.CityID = Village.CityID 

--3. Display the city having more than one village. 

		SELECT CITY.Name FROM CITY JOIN Village ON 
		City.CityID = Village.CityID 
		GROUP BY City.Name
...
--4. Display the city having no village. 
		...
		SELECT CITY.Name,City.CityID FROM CITY LEFT OUTER JOIN Village ON 
		City.CityID = Village.CityID 
		WHERE Village.CityID IS NULL;

--5. Count the total number of villages in each city. 

		SELECT COUNT(Village.CityID),City.Name FROM City JOIN Village ON
		City.CityID = Village.CityID 
		GROUP BY City.Name;
...
--6. Count the number of cities having more than one village.
		...
		SELECT COUNT(City.CityID) FROM City WHERE City.Name IN
